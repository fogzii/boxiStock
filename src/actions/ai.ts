...
const stockSchema: Schema = {
  ...
  items: {
    ...
    properties: {
      ...
      lotIdentity: {
        type: SchemaType.STRING,
        description: "Optional lot identity, identifier, or condition (e.g. '1st edition', 'mint', 'batch A')"
      },
      dateAcquired: {
        type: SchemaType.STRING,
        description: "Acquisition or received date (YYYY-MM-DD). If not mentioned, ALWAYS default to the current date provided in the prompt."
      }
    },
    required: ["name", "initialQuantity", "buyPrice", "isStocked", "dateAcquired"]
  }
};
